...
echo '[2/14] Ubuntu 26.04 source lock'
...
echo '[3/14] APT live-media source safety'
...
echo '[4/14] Theme/Plymouth byte lock'
...
echo '[5/14] No legacy immutable runtime path'
...
echo '[6/14] Installer contrast safety'
...
echo '[7/14] Independent updater coverage'
python3 - <<'PY'
import json
from pathlib import Path
...
echo '[8/14] Gaming/Deskflow stack'
...
echo '[9/14] Mail/Klang user-update launchers'
...
echo '[10/14] Update ZIP builder smoke test'
...
echo '[11/14] Ubuntu 26.04 required-package compatibility guards'
...
echo '[12/14] GDM usr-merge/display-manager safety'
...
echo '[13/14] GDM resource target detection safety'
...
echo '[14/14] GitHub build wiring'
...
grep -q '3.0.0-starter1-fix8' VERSION || fail 'FIX8 version marker missing'
