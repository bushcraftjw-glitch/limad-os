...
echo '[2/13] Ubuntu 26.04 source lock'
...
echo '[3/13] APT live-media source safety'
...
echo '[4/13] Theme/Plymouth byte lock'
...
echo '[5/13] No legacy immutable runtime path'
...
echo '[6/13] Installer contrast safety'
...
echo '[7/13] Independent updater coverage'
python3 - <<'PY'
import json
from pathlib import Path
...
echo '[8/13] Gaming/Deskflow stack'
...
echo '[9/13] Mail/Klang user-update launchers'
...
echo '[10/13] Update ZIP builder smoke test'
...
echo '[11/13] Ubuntu 26.04 required-package compatibility guards'
...
echo '[12/13] GDM usr-merge/display-manager safety'
...
echo '[13/13] GitHub build wiring'
...
grep -q '3.0.0-starter1-fix7' VERSION || fail 'FIX7 version marker missing'
