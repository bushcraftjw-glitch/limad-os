...
echo '[2/12] Ubuntu 26.04 source lock'
...
echo '[3/12] APT live-media source safety'
...
echo '[4/12] Theme/Plymouth byte lock'
...
echo '[5/12] No legacy immutable runtime path'
...
echo '[6/12] Installer contrast safety'
...
echo '[7/12] Independent updater coverage'
python3 - <<'PY'
import json
from pathlib import Path
...
echo '[8/12] Gaming/Deskflow stack'
...
echo '[9/12] Mail/Klang user-update launchers'
...
echo '[10/12] Update ZIP builder smoke test'
...
echo '[11/12] Ubuntu 26.04 required-package compatibility guards'
...
grep -q '11-apt-required-preflight.sh' build_files/customize-rootfs.sh || fail 'APT required-package preflight not wired'
 ...
 echo '[12/12] GitHub build wiring'
...
grep -q '3.0.0-starter1-fix6' VERSION || fail 'FIX6 version marker missing'
